...
const verifyEmail = catchError(async(req, res) => {
    ...
    if(!emailCode) return res.status(401).json({ message: "Nooo asi no es"});
    ...
    const login = catchError(async(req, res) => {
        ...
        if (!user) return res.status(401).json({message: "Credenciales incorrectos"});
        const isValid = await bcrypt.compare(password, user.password);
        if (!isValid) return res.status(401).json({message: "Credenciales incorrectos"});
        if (user.isVerified === false) return res.status(401).json({message: "Verifica tu correo"})
